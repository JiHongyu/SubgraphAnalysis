...
class PyLouvain:
    ...
    def from_file(cls, path):
        ...
        nodes_, edges_, _ = in_order(nodes, edges)
    ...
    def from_data(cls, nodes, edges):
        nodes, edges, nodes_map = in_order(nodes, edges)
        #print("%d nodes, %d edges" % (len(nodes), len(edges)))
        return cls(nodes, edges,nodes_map)   
            
    '''
        Initializes the method.
        _nodes: a list of ints
        _edges: a list of ((int, int), weight) pairs
        node_map: 这是我加的参数，算法中处理的是节点索引值，节点名和节点索引值需要映射
    '''
    def __init__(self, nodes, edges, node_map=None):
        #  迭代不变量
        self.nodes = nodes
        self.edges = edges
    ...
    def apply_method(self):
        ...
        act_partition_idx = self.actual_partition
        act_partition_name = []
        for partition in act_partition_idx:
            act_partition_name.append([self.node_map[x] for x in partition])
            
        return (act_partition_idx,act_partition_name, best_q)
    ...
    def first_phase(self, network):
        ...
        # best_partition is a list 
        best_partition = self.make_initial_partition(network)
        while 1:
            improvement = 0
            for node in network[0]:
                node_community = self.communities[node] # 社团索引号
                ...
                best_shared_links = 0 # 节点社团内邻居边权值和
...
'''
    ...
    _edges: a list of ((int, int), weight) pairs
    _dc   : a map of nodes index to names
'''
def in_order(nodes, edges):
        ...
        return (nodes_, edges_,dc_)

# 内部函数
def nx2pylouvain(nxNet):
    '''
    作用：networkx.Graph对象转换为PyLouvain对象
    参数：nxNet：Networkx.Graph对象
    输出：pyl：PyLouvain对象
    '''
    nodes = {}
    edges = nxNet.edges(data='weight', default=1)
    for node in nxNet.nodes():
        nodes[node] = 1
    edges = [((x[0],x[1]),x[2]) for x in edges]
    pyl = PyLouvain.from_data(nodes, edges)
    return pyl

# 接口函数
def LouvainCommunities(nxNet):
    '''
    作用：基于PyLouvain的BGLL社团发现
    参数：nxNet：Networkx.Graph对象
    
    输出: 
    Communities --- 社团格式(假设共k个社团): [[c1],[c2],...,[ck]]
    Modularity --- 当前划分的模块度
    '''
    pyl = nx2pylouvain(nxNet)
    idx, Communities, Modularity = pyl.apply_method()
